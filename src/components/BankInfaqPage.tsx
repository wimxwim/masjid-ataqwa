...
import { useDefaultMosque, usePublicTestimonials } from "@/lib/queries/public";

export default function BankInfaqPage() {
  const { appToast, triggerToast } = useAppContext();
  const router = useRouter();
  const { data: mosque } = useDefaultMosque();
  const { data: testimonials = [] } = usePublicTestimonials(mosque?.id ?? "");
